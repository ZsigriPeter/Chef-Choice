...
function getStartDateOfCurrentWeek() {
  ...
  return new Date(
    currentDate.setDate(currentDate.getDate() - (currentDayIndex - 1))
  );
}

function getStartDateStringOfWeek(actualDate) {
  return `${actualDate.getUTCFullYear()}-${(actualDate.getMonth() + 1)
    .toString()
    .padStart(2, "0")}-${actualDate.getUTCDate()}`;
}

function WeeklyMenuPage() {
  const [menu, setMenu] = useState(null);
  const [currentWeekStartDate, setCurrentWeekStartDate] = useState(
    getStartDateStringOfWeek(getStartDateOfCurrentWeek())
  );
  ...
  }, [searchParams, currentWeekStartDate]);

  return (
    <div className="weeklyMenuPage">
      <SideMenuBar />
      {menu && (
